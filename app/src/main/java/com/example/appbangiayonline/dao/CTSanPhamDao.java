...
public class CTSanPhamDao {
    ...
    public boolean checkCTSP(String mausac, int kichco) {
        SQLiteDatabase sql = dbHelper.getWritableDatabase();
        Cursor cursor = sql.rawQuery("select magiohang from ctsanpham where mausac=? and kichco=?", new String[]{mausac, String.valueOf(kichco)});
        if (cursor.getCount() > 0) {
            return true;
        }
        return false;
